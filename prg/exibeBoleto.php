...
$model->define_templates( array( 'bolonline' => Modelo($DirModelos, 'boleto') ));
...
if (substr($Codigo, 0, 1) == 'C')
	$Codigo = intval(substr($Codigo, 1));
else
...
//echo "\n[$sLogo] -->\n";
		...
		{
			// No XML deve usar o valor obtido retirando apenas CR/LF.
			if ($Tag == 'IsId1')
			{
				if ($sTipo == 'C')
				{
...
if (!$bQuitado)
{
	if (trim($sRetido) == 'S')
	{
		Mensagem('Aviso', 'Boleto retido ou cancelado. Favor contactar a Imobiliária!');
		exit;
	}
	...
	if (strstr($filename, '.sempre.txt') != '.sempre.txt')
	{
		...
		$sData = substr($sVencto, 6, 4) . '-' . substr($sVencto, 3, 2) . '-' . substr($sVencto, 0, 2);
		$dtData = new DateTime($sData);
		$iSemana =  $dtData->format("w");
		// Verifica se vencimento cai no fim de semana
		if($iSemana == 6){
			$iExpiracaoBloq += 2 * 86400; // Se venceu no sabado, dah mais 2 dias
		}
		else if ($iSemana == 0){
			$iExpiracaoBloq += 86400;	  // Se venceu no domingo, dah mais 1 dias
		}

//printf("<!-- Tipo='%s' Vencto=%s Expira=%d -->\n", $sTipo, $iTempoVencido, $iExpiracaoBloq);
		if ($iTempoVencido > $iExpiracaoBloq)
		{
			Mensagem('Aviso', "Este boleto possui vencimento em '$sVencto' e já expirou. Favor contactar a Imobiliária!");
			return;
		}
	}
}
...
else 
{
	if ($ExibirPdf && !headers_sent() && file_exists('mpdf/mpdf.php'))
	{
		try {
			...
			$mpdf=new mPDF('pt_BR','A4'); 
			$mpdf->WriteHTML($html);
			$mpdf->Output('','S');
			ob_end_clean();
			// Tudo certo entao exibe o PDF
			$mpdf->Output('DOC-'.ISO8859_1toASCII(str_replace(' ','_',$sSacado)).'.pdf','I');
			return;
		} catch(Exception $e) { 
			ob_end_clean();
		}
