...
if ($Prod == 'S')
{
	...
	if (empty($Modelo))
		$Modelo = 'extratoloc';
	$model->define_templates( array ( 'extratoloc' => Modelo($DirModelos, $Modelo) )); 
...
NewLine;
	...
	$model-> define_templates( array ('demonstrativo' => Modelo($DirModelos, $Modelo) )); 
	$model->define_dynamic('EXTRATO', 'demonstrativo');	//body is the parent of table
	$model->define_dynamic('IMOVEL', 'demonstrativo');	//body is the parent of table
	$model->define_dynamic('IMOVEL_ITEM', 'IMOVEL');	//body is the parent of table
	$model->define_dynamic('TOTAIS', 'demonstrativo');	//body is the parent of table
	$model->define_dynamic('POR_DIA', 'demonstrativo');	//body is the parent of table
	...
	$model->parse('demonstrativo');
	$model->DPrint('demonstrativo');
}
...
NewLine;
	...
	if ($UsingXmlModel)
	{
		...
		$model->define_dynamic('EXIBE_RESUMO_REC', $Modelo);
		$model->define_dynamic('RESUMO_REC', 'EXIBE_RESUMO_REC');
		$model->define_dynamic('EXIBE_RESUMO_ENT', $Modelo);
...
//echo " Conselheiro:".$bConselheiro."-->\n";
		...
		else if ($TipoReg == 'T')	// Resumo de Taxas (despesas)
		{
			$model->parse('.RESUMO'); 
			$bTemResumo = true;
		}
		else if ($TipoReg == 'R')	// Resumo de Taxas (receitas)
		{
