...
//AddMsg("NormalizePath($path) => '$newpath'<br>\n");
...
function HttpPath($path)
{
	...
	$localpath = NormalizePath($Http_dir_base.'/'.$Http_dir.'/'.$path);
...
function FtpInit()
{
	...
	if (isset($_REQUEST['ftpdir']))
		$Ftp_dir = NormalizePath($_REQUEST['ftpdir']);
...
function FtpPath($path)
{
	...
	if (empty($Ftp_dir) && empty($path))
		...
		$localpath = NormalizePath('/'.$Ftp_dir.'/'.$path);
...
//AddMsg("MkPath($newdir)<br>\n");
	...
	{
		if ($Http_dir{0} == '/')
			$newdir = NormalizePath(substr($Http_dir,1).'/'.$newdir);
		else
			$newdir = NormalizePath($Http_dir.'/'.$newdir);
	}
...
//AddMsg(" Verificando '$newdir'<br>\n");
	...
	foreach($parts as $part){
//AddMsg(" Criando '$part':<br>\n");
		if (empty($part) || @file_exists($part))
			continue;
		if (@mkdir($part) === false)
			return false;
...
else if ($oper == "tstftp")
{
	...
	$ftpfile = NormalizePath(dirname($srcfile).'/tstftp.tmp');
...
else if ($oper == "save")
{
	...
	$destdir = NormalizePath(dirname($file));
	if (!MkPath($destdir))
