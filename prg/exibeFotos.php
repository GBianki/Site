...
//echo "\n-->\n";
		reset($files);
		$cont = 0;
		while (list($file, $ordem) = each($files))
		{
			if ($exibeThumbNails)
			{
...
if ($exibeEnder && $GoogleMapsKey != '')
{
	...
	$model->assign('GOOGLEMAPS_INC', sprintf("<script src=\"http://maps.google.com/maps?file=api&amp;v=2&amp;sensor=false&amp;key=%s\" type=\"text/javascript\"></script>", $GoogleMapsKey));
