...
for(;;)
{
	...
	else if ($TipoReg == 'I')	// Informacoes
	{
		...
		if ($sTipo == 'EMAI')
		{
			...
			{
				if ($UsingXmlModel)
					$sOutrasInfos .= sprintf("   <email>%s</email>\n", ISO8859_1toModel($sEmail));
				else
	...
	if (isset($sFone1) && strlen($sFone1) > 1)
	{
		...
		{
			if ($UsingXmlModel)
				$sFones .= sprintf("    <fone>%s%s</fone>\n", ISO8859_1toModel($sFone1), (strlen($sRama11) > 1 ? 'Ramal '.$sRama11 : ''));
			else
			{
				$model->assign('UNID', $sUnidade);
				$sDescr = '<td valign="top" colspan=2>FONE: '.$sFone1.(strlen($sRama11) > 1 ? 'Ramal '.$sRama11 : '').'</td>';
				$model->assign('DESCR', $sDescr);
				$model->assign('EXIBE_SEPARADOR', empty($sUnidade) ? 'style="visibility:hidden"' : '');
				$model->parse($iCont++ == 0 ? 'CONDONIMO' : '.CONDONIMO');
				$sUnidade = '';
	...
	if (isset($sFone2) && strlen($sFone2) > 1)
	{
		...
		{
			if ($UsingXmlModel)
				$sFones .= sprintf("    <fone>%s%s</fone>\n", ISO8859_1toModel($sFone2), (strlen($sRama12) > 1 ? ' Ramal '.$sRama12 : ''));
			else
			{
				$model->assign('UNID', $sUnidade);
				$sDescr = '<td valign="top" colspan=2>FONE: '.$sFone2.(strlen($sRama12) > 1 ? 'Ramal '.$sRama12 : '').'</td>';
				$model->assign('DESCR', $sDescr);
				$model->assign('EXIBE_SEPARADOR', empty($sUnidade) ? 'style="visibility:hidden"' : '');
				$model->parse($iCont++ == 0 ? 'CONDONIMO' : '.CONDONIMO');
				$sUnidade = '';
