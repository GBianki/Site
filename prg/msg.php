...
include dirname(__FILE__) . '/class.DTemplate.php';
...
if( !isset($_SESSION['SESSION_VERSION']) || $_SESSION['SESSION_VERSION'] != $configIni['session_version']) {
	$config_file = '';
}
if( strpos($_SESSION['_CONF_DIR_MODELOS'], '/var') === FALSE ||  strpos($_SESSION['_CONF_DIR_MODELOS'], '/var') !== 0) {
	$config_file = '';
}
if (empty($config_file))
{
		$_SESSION['SESSION_VERSION'] = $configIni['session_version'];
	...
	for ($i = 0; $i < 2; $i++)
	{
		...
		case 0:
			...
			else if (is_file(dirname(__FILE__) . '/config.inc-dist.php'))
				$config_file = dirname(__FILE__) . '/config.inc-dist.php';
			else
			{
			...
			break;
		case 1:
			if (!is_file(dirname(__FILE__) . '/bloqueto.inc.php'))
				continue;
			$config_file = dirname(__FILE__) . '/bloqueto.inc.php';
			break;
		}
...
function GetFullUrl($path)
{
	...
	return 'http://'.NormalizePath($_SERVER["SERVER_NAME"].'/'.$path);
}
...
<H3>%s</H3>
	...
	$sHttpHost = 'http://'.$_SERVER['HTTP_HOST'].'/'.substr($sPathSite, 0, strpos($sPathSite, '/')).'/';
