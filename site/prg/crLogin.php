...
function ShowServices(&$model, &$aServicos)
{
	...
	{
		$model->assign('PRODUTO', $aServ['PRODUTO']);
		$model->assign('PROD', $aServ['PROD']);
		$model->assign('DESCR', $aServ['DESCR']);
		$model->assign('DESC_SERV', $aServ['DESC_SERV']);
				...
				if ($TipoBloco == 'N')
				{
						...
						if (($File=@fopen($DirDados.$UserFile, 'r')) !== false)
						{
							...
							if (substr($sReg, 0, 1) == 'S')
							{
								...
								if ($bConselheiro)
								{
									...
									$OpcoesExtras[] = array(
										'PRODUTO'=>ISO8859_1toModel('Inadimplência'), 
										'PROD'=>'I',
										'DESCR'=>ISO8859_1toModel($Descr), 
										'DESC_SERV'=>'',
										'DESCR_PARAM'=>'',
										'CHAVE'=>$Chave,
					...
					if ($ExibeResumos && $TipoProd == 'C' && $Condomino == 'S' && $CodCondom != $UltResumo)
					{
						...
						if ($arq !== false)
						{
							...
							$OpcoesExtras[] = array(
								'PRODUTO'=>ISO8859_1toModel('Resumo de DOCs'), 
								'PROD'=>'RDOC',
								'DESCR'=>ISO8859_1toModel(trim($descrResumo)), 
								'DESC_SERV'=>'',
								'DESCR_PARAM'=>'',
								'CHAVE'=>$Chave,
						...
						if ($arq !== false)
						{
							...
							$OpcoesExtras[] = array(
								'PRODUTO'=>ISO8859_1toModel('Resumo de Receitas e Despesas'), 
								'PROD'=>'RRD',
								'DESCR'=>ISO8859_1toModel(trim($descrResumo)), 
								'DESC_SERV'=>'',
								'DESCR_PARAM'=>'',
					...
					if ($ExibeAnexos && $TipoProd == 'C' && $CodCondom != $UltAnexo)
					{
						...
						if (is_dir($DirAnexosCond))
						{
							...
							$OpcoesExtras[] = array(
								'PRODUTO'=>ISO8859_1toModel('Documentos do Condomínio (atas, convocações, etc.)'), 
								'PROD'=>'C',
								'DESCR'=>ISO8859_1toModel(trim($Descr)), 
								'DESC_SERV'=>'',
...
//echo "<!-- UltChave=$UltPlanilha -->\n";
					...
					if ($ListaCondominos && $TipoProd == 'C' && $Condomino == 'S' && $CodCondom != $UltLista)
					{
						...
						if ($arq !== false)
						{
							...
							$OpcoesExtras[] = array(
								'PRODUTO'=>ISO8859_1toModel('Lista de Condôminos'), 
								'PROD'=>'L',
								'DESCR'=>ISO8859_1toModel($Descr), 
								'DESC_SERV'=>'',
								'DESCR_PARAM'=>'',
